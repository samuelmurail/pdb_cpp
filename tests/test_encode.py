...
"""
Tests for hybrid-36 encode/decode.
"""

from pdb_cpp import core as encode


def test_hy36_roundtrip():
    def recycle4(value, encoded):
        s = encode.hy36encode(width=4, value=value)
        assert s == encoded
        d = encode.hy36decode(width=4, value=s)
        assert d == value

    assert encode.hy36decode(width=4, value="    ") == 0
    assert encode.hy36decode(width=4, value="  -0") == 0
    recycle4(-999, "-999")
    recycle4(-78, " -78")
    recycle4(-6, "  -6")
    recycle4(0, "   0")
    ...
    def recycle5(value, encoded):
        ...
        assert s == encoded
        d = encode.hy36decode(width=5, value=s)
        assert d == value

    assert encode.hy36decode(width=5, value="     ") == 0
    assert encode.hy36decode(width=5, value="   -0") == 0
    recycle5(-9999, "-9999")
    recycle5(-123, " -123")
    recycle5(-45, "  -45")
    recycle5(-6, "   -6")
    ...
    for width, ss in [
        ...
        for s in ss:
            try:
                encode.hy36decode(width, value=s)
            except (ValueError, RuntimeError) as exc:
