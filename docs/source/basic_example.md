...
```python
coor_1hsg = pdb_cpp.Coor('./1hsg.pdb')
```
...
```python
coor_1hsg = pdb_cpp.Coor('./1hsg.cif')
```
...
The obtained selection can be saved using the `write()` function:

```python
# Save the ligand coordinates
lig_coor.write('1hsg_lig.pdb')
```
